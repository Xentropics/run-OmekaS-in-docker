...
for ((i = 0; i < $MODULE_COUNT; i++)); do
    ...
    if $MODULE_INSTALL; then
        ...
        #curl -v -L -o /tmp/$MODULE_NAME.zip $MODULE_URL
        wget --quiet --output-document="/tmp/${MODULE_NAME}.zip" "$MODULE_URL"
        ...
        for ((j = 0; j < $PREACTIVATECOUNT; j++)); do
        ...
        for ((j = 0; j < $POSTINSTALLCOUNT; j++)); do
